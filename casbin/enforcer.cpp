...
void Enforcer :: __construct(Php::Parameters &parms) {
    string model_path_ = parms[0];
    string policy_file_ = parms[1];

    this->temp = new Enforcer(model_path_, policy_file_);
}
...
Php::Value Enforcer::Enforce(Php::Parameters &params) {
    vector<string> par = params[0];
    return this->temp->Enforce(par);
}
...
// managemet_api and internal_api common API
Php::Value Enforcer :: addPolicy(Php::Parameters &params) {
    if (params.size() == 1) {
        vector<string> par = params[0];
        return this->temp->AddPolicy(par);
    } else if (params.size() == 3) {
        string sec = params[0];
        string p_type = params[1];
        vector<string> rule = params[2];
        return this->temp->AddPolicy(sec, p_type, rule);
    }
}

Php::Value Enforcer :: addPolicies(Php::Parameters &params) {
    if (params.size() == 1) {
        vector<vector<string>> par = params[0];
        return this->temp->AddPolicies(par);
    } else if (params.size() == 3) {
        string sec = params[0];
        string p_type = params[1];
        vector<vector<string>> rule = params[2];
        return this->temp->AddPolicies(sec, p_type, rule);
    }
}

Php::Value Enforcer :: removePolicy(Php::Parameters &params) {
    if (params.size() == 1) {
        vector<string> par = params[0];
        return this->temp->RemovePolicy(par);
    } else if (params.size() == 3) {
        string sec = params[0];
        string p_type = params[1];
        vector<string> rule = params[2];
        return this->temp->RemovePolicy(sec, p_type, rule);
    }
}

Php::Value Enforcer :: removePolicies(Php::Parameters &params) {
    if (params.size() == 1) {
        vector<vector<string>> par = params[0];
        return this->temp->RemovePolicies(par);
    } else if (params.size() == 3) {
        string sec = params[0];
        string p_type = params[1];
        vector<vector<string>> rule = params[2];
        return this->temp->RemovePolicies(sec, p_type, rule);
    }
}

Php::Value Enforcer :: removeFilteredPolicy(Php::Parameters &params) {
    if (params.size() == 2) {
        int field_index = params[0];
        vector<string> par = params[1];
        return this->temp->RemoveFilteredPolicy(field_index, par);
    } else if (params.size() == 4) {
        string sec = params[0];
        string p_type = params[1];
        int field_index = params[2];
        vector<string> rule = params[3];
        return this->temp->RemoveFilteredPolicy(sec, p_type, field_index, rule);
    }
}
...
{
    enforcer.method<&Enforcer::__construct>("__construct");
    enforcer.method<&Enforcer::Enforce>("Enforce");
}

void registerCommonApi(Php::Class<Enforcer> &enforcer)
{
    enforcer.method<&Enforcer::addPolicy>("addPolicy");
    enforcer.method<&Enforcer::addPolicies>("addPolicies");
    enforcer.method<&Enforcer::removePolicy>("removePolicy");
    enforcer.method<&Enforcer::removePolicies>("removePolicies");
    enforcer.method<&Enforcer::removeFilteredPolicy>("removeFilteredPolicy");
}